...
export const getServiceTestimonials = cache((service: string) => {
  ...
  const testimonialsFromData = getTestimonialsByService(service).map(testimonial => ({
    ...testimonial,
    location: testimonial.location || "Calgary, AB",
  }));
  ...
  const serviceObject = services.find(s => 
    s.title === service || 
    s.slug === service || 
    s.id === service
  );
...
export const getDataMetrics = cache(() => {
  ...
  return {
    ...
    featuredOffers: consolidatedOffers.filter(o => o.badges.includes('Best Seller')).length,
 ...
 */
export const getRelatedServices = cache((currentSlug: string, limit: number = 3): Service[] => {
  ...
  if (currentService.relatedServiceIds && currentService.relatedServiceIds.length > 0) {
    return services
      .filter(service => currentService.relatedServiceIds.includes(service.id))
      .slice(0, limit);
  }
