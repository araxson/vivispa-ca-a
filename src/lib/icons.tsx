import React, { type SVGProps } from 'react';
import type { LucideIcon } from 'lucide-react';
import {
  // Navigation & Actions
  ArrowRight,
  ArrowLeft,
  ChevronRight,
  ChevronDown,
  ChevronUp,
  ExternalLink,
  Home,
  Menu,
  X,
  XIcon,
  
  // UI & Feedback
  Search,
  SearchX,
  Filter,
  Check,
  CheckIcon,
  Circle,
  CircleIcon,
  AlertCircle,
  Heart,
  Star,
  Smile,
  
  // Location & Contact
  MapPin,
  Phone,
  Mail,
  Calendar,
  Clock,
  
  // Social & Communication
  Instagram,
  Facebook,
  Twitter,
  Youtube,
  
  // Theme & Settings
  Moon,
  Sun,
  
  // Content & Media
  Tags,
  Tag,
  TrendingDown,
  MoreHorizontal,
  
  // Help & Support
  HelpCircle,
} from 'lucide-react';

/**
 * Centralized Icon System
 * 
 * This module provides a unified approach to icon management across the application.
 * It includes:
 * - Centralized Lucide icon exports
 * - Custom SVG components
 * - Icon mapping utilities
 * - Consistent sizing and color systems
 */

// =============================================================================
// Custom SVG Icons
// =============================================================================

/**
 * WhatsApp icon component
 */
export function WhatsAppIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 448 512"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157.2 341.6c-33.8 0-67.6-9.2-97.2-26.7l-7-4.1-68.8 18.1 18.3-67.1-4.5-7.2c-19.1-30.4-29.7-65.7-29.7-101.8 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z" />
    </svg>
  );
}

// =============================================================================
// Icon Collections & Mappings
// =============================================================================

/**
 * Commonly used Lucide icons organized by category
 */
export const IconCollection = {
  // Navigation & Actions
  ArrowRight,
  ArrowLeft,
  ChevronRight,
  ChevronDown,
  ChevronUp,
  ExternalLink,
  Home,
  Menu,
  X,
  XIcon,
  
  // UI & Feedback
  Search,
  SearchX,
  Filter,
  Check,
  CheckIcon,
  Circle,
  CircleIcon,
  AlertCircle,
  Heart,
  Star,
  Smile,
  
  // Location & Contact
  MapPin,
  Phone,
  Mail,
  Calendar,
  Clock,
  
  // Social & Communication
  Instagram,
  Facebook,
  Twitter,
  Youtube,
  
  // Theme & Settings
  Moon,
  Sun,
  
  // Content & Media
  Tags,
  Tag,
  TrendingDown,
  MoreHorizontal,
  
  // Help & Support
  HelpCircle,
} as const;

/**
 * Icon mappings for specific use cases
 */
export const IconMaps = {
  filter: {
    location: MapPin,
    category: Filter, 
    search: Search,
    tag: Tag,
  },
  social: {
    whatsapp: WhatsAppIcon,
    instagram: Instagram,
    facebook: Facebook,
    twitter: Twitter,
    youtube: Youtube,
  },
  navigation: {
    home: Home,
    back: ArrowLeft,
    forward: ArrowRight,
    external: ExternalLink,
    menu: Menu,
    close: X,
  },
  feedback: {
    success: Check,
    error: X,
    warning: AlertCircle,
    info: AlertCircle,
    favorite: Heart,
    rating: Star,
  },
  contact: {
    location: MapPin,
    phone: Phone,
    email: Mail,
    calendar: Calendar,
    time: Clock,
  },
} as const;

/**
 * Icon size classes for consistent sizing */
export const iconSizes = {
  xs: "h-3 w-3",
  sm: "h-4 w-4", 
  md: "h-5 w-5",
  lg: "h-6 w-6",
  xl: "h-8 w-8",
  "2xl": "h-10 w-10",
  "3xl": "h-12 w-12",
} as const;

/**
 * Icon color variants
 */
export const iconColors = {
  default: "text-foreground",
  muted: "text-muted-foreground",
  primary: "text-primary",
  destructive: "text-destructive",
  success: "text-green-600", 
  warning: "text-yellow-600",
  info: "text-blue-600",
} as const;

/**
 * Trend indicators for statistics
 */
export const TrendIcons = {
  up: "↗",
  down: "↘", 
  neutral: "→",
} as const;

// =============================================================================
// Type Definitions
// =============================================================================

export type IconSize = keyof typeof iconSizes;
export type IconColor = keyof typeof iconColors;
export type IconName = keyof typeof IconCollection;

// =============================================================================
// Utility Functions
// =============================================================================

/**
 * Get icon class names for size and color
 */
export function getIconClasses(size: IconSize = 'md', color: IconColor = 'default'): string {
  return `${iconSizes[size]} ${iconColors[color]}`;
}

/**
 * Get icon from the centralized collection
 */
export function getIcon(name: IconName): LucideIcon {
  return IconCollection[name];
}

// =============================================================================
// Re-exports for Backward Compatibility
// =============================================================================

// Re-export commonly used icons directly for convenience
export {
  ArrowRight,
  ArrowLeft,
  ChevronRight,
  ChevronDown,
  ChevronUp,
  ExternalLink,
  Home,
  Menu,
  X,
  XIcon,
  Search,
  SearchX,
  Filter,
  Check,
  CheckIcon,
  Circle,
  CircleIcon,
  AlertCircle,
  Heart,
  Star,
  Smile,
  MapPin,
  Phone,
  Mail,
  Calendar,
  Clock,
  Instagram,
  Facebook,
  Twitter,
  Youtube,
  Moon,
  Sun,
  Tags,
  Tag,
  TrendingDown,
  MoreHorizontal,
  HelpCircle,
};
