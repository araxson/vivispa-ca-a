...
export const getItemById = cache(
  <T>(source: DataSource, id: string): T | undefined => {
    // Check if the source exists
    const dataSource = dataSources[source] || {};
    if (Object.keys(dataSource).length === 0) {
      console.warn(`Data source '${source}' is empty or not found`);
    ...
    if (
      source === "services" &&
      typeof dataSource.getServiceById === "function"
    ) {
...
export const getItemBySlug = cache(
  <T>(source: DataSource, slug: string): T | undefined => {
    // Check if the source exists
    const dataSource = dataSources[source] || {};
    if (Object.keys(dataSource).length === 0) {
      console.warn(`Data source '${source}' is empty or not found`);
    ...
    if (
      source === "services" &&
      typeof dataSource.getServiceBySlug === "function"
    ) {
 ...
 */
export const filterItems = cache(
  <T>(source: DataSource, predicate: (item: T) => boolean): T[] => {
    try {
...
export const getFeaturedItems = cache(<T>(source: DataSource): T[] => {
  ...
  if (
    source === "services" &&
    "featuredServices" in dataSource &&
    Array.isArray(dataSource.featuredServices)
  ) {
