...
function getServiceCategoryName(offer: SmartOfferServiceItem): string {
  const categoryConfig = ALL_SERVICE_CATEGORIES.find(category => 
...
export function OfferCard({ offer }: OfferCardProps) {
  ...
  const discountPercentage = offer.pricing.isSpecialOffer && offer.pricing.originalPrice 
    ? calculateDiscountPercentage(offer.pricing.originalPrice, offer.pricing.display)
    : 0;
  ...
  const getAllLocationsWithAvailability = () => {
    ...
    return allLocationNames.map(locationName => {
      const isAvailable = offer.allAvailableLocations?.some(
      ...
      return {
        ...
        address: locations.find(loc => loc.name === locationName)?.address || ''
      };
    });
  };
  ...
  return (
    <Card className="border border-border bg-card overflow-hidden rounded-lg h-full flex flex-col">
      ...
      <div className="relative h-36 overflow-hidden flex-shrink-0 group">
        <Image
            ...
            {offer.pricing.isSpecialOffer && offer.pricing.originalPrice && (
              <div className="text-right">
                <div className="text-xs font-bold text-success flex items-center gap-0.5">
                  ...
                  Save ${(
                    parseFloat(offer.pricing.originalPrice.replace(/[^0-9.]/g, '')) - 
            ...
            !location.isAvailable ? 'opacity-50 cursor-not-allowed' : ''
                ...
                >
                  <input
                    ...
                    onChange={(e) => location.isAvailable && setSelectedCardLocation(e.target.value)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`font-medium text-xs ${
                      location.isAvailable ? 'text-foreground' : 'text-muted-foreground'
                    }`}>
                      {location.name}
                      {!location.isAvailable && (
                        <span className="ml-1 text-xs text-destructive font-normal">(Not Available)</span>
                      )}
                    </div>
                    <div 
                      id={`location-${offer.id}-${index}-description`}
                      className={`text-xs mt-0.5 leading-tight ${
                        location.isAvailable ? 'text-muted-foreground' : 'text-muted-foreground/70'
                      }`}
                    >
                      {location.address}
                    </div>
