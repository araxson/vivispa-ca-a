...
export function PricingCategorySection({ 
  ...
  const hasSubcategories = Object.keys(servicesBySubcategory).length > 1 || !servicesBySubcategory['main'];
  ...
  return (
    <div className="bg-card rounded-lg border">
      <div className="p-6 border-b border-border">
        ...
        >
          <div>
            <h2 className="text-2xl font-bold text-foreground mb-1">{category}</h2>
            <p className="text-sm text-muted-foreground">{services.length} service{services.length !== 1 ? 's' : ''} available</p>
          </div>
          {expanded ? (
        ...
        <div className="p-6">
          {hasSubcategories ? (
            <div className="space-y-8">
              {Object.entries(servicesBySubcategory).map(([subcategory, subcategoryServices]) => (
                <div key={subcategory}>
                  {subcategory !== 'main' && (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {subcategoryServices.map((service, index) => (
                      <PricingCard key={`${service.name}-${index}`} service={service} />
                    ))}
                  </div>
                </div>
              ))}
