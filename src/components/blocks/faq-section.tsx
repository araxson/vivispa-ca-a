      ...
      <Section spacing="lg" className={className}>
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            {displayFaqs.map((faq, index) => (
              <div key={faq.id || index} className="border-b border-border pb-6 last:border-b-0">
      ...
      <Section spacing="lg" className={className}>
        ...
        <div className="max-w-3xl mx-auto">
          <Accordion type="single" collapsible className="w-full space-y-4">
            {displayFaqs.map((faq, index) => (
              <AccordionItem key={faq.id || index} value={`item-${index}`} className="border rounded-lg bg-card overflow-hidden">
          ...
          {groupedFAQs.map(([category, faqs]) => (
            <div key={category} className="space-y-4">
              ...
              <Accordion type="single" collapsible className="w-full space-y-3">
                {faqs.map((faq, index) => (
                  <AccordionItem key={faq.id || index} value={`${category}-${index}`} className="border rounded-lg bg-card overflow-hidden">
                    <AccordionTrigger className="px-6 py-4 text-left hover:no-underline hover:bg-muted/50 transition-colors">
                      <span className="font-semibold text-foreground pr-4">{faq.question}</span>
                    </AccordionTrigger>
                    <AccordionContent className="px-6 pb-4 pt-0">
