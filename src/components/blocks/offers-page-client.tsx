...
export const OffersPageClient = memo(function OffersPageClient({
  ...
  const availableCategories = useMemo(() => {
    return SERVICE_CATEGORIES.map((category) => {
      let categoryOffers = initialOffers.filter(
        (offer) =>
  ...
  const filteredOffers = useMemo(() => {
    let processedOffers: SmartOfferItem[] = initialOffers.map((offer) => {
      if (selectedLocation) {
        const locationData = offer.availableLocations?.find(
          (loc) => loc.location === selectedLocation,
        );
        if (locationData) {
    ...
    if (selectedLocation) {
      processedOffers = processedOffers.filter(
        (offer) => offer.isAvailableAtSelectedLocation,
      );
    }
      ...
      if (categoryConfig) {
        processedOffers = processedOffers.filter(
          (offer) =>
            ...
            offer.name
              .toLowerCase()
              .includes(categoryConfig.name.toLowerCase()) ||
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          ...
          >
            ...
            <SelectContent>
              ...
              {availableCategories.map((category) => (
                ...
                >
                  {category.name}{" "}
                  {category.offerCount > 0 && `(${category.offerCount})`}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          <Select value={sortBy} onValueChange={setSortBy}>
            <SelectTrigger className="w-full" aria-label="Sort offers">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
