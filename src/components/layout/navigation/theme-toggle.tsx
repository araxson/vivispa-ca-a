...
export function ThemeToggle() {
  ...
  if (!mounted) {
    return (
      <div className="w-11 h-6 bg-muted/30 rounded-full animate-pulse" aria-hidden="true" />
    )
  }
  ...
  return (
    ...
    >
      ...
      >
        {theme === "dark" ? (
          <Moon size={12} className="text-slate-600 transition-colors duration-300" />
        ) : (
          <Sun size={12} className="text-amber-600 transition-colors duration-300" />
        )}
      </SwitchPrimitive.Thumb>
    </SwitchPrimitive.Root>
  )
