...
export const DEFAULT_IMAGE = '/images/services/hydrofacial/hydrofacial-in-calgary-001.webp';
...
const SERVICE_IMAGES = {
  ...
  HYDROFACIAL_EXPRESS: '/images/services/hydrofacial/hydrofacial-in-calgary-003.webp',
  HYDROFACIAL_DELUXE: '/images/services/hydrofacial/hydrofacial-in-calgary-002.webp',
  HYDROFACIAL_ANTI_AGING: '/images/services/hydrofacial/hydrofacial-in-calgary-001.webp',
  HYDROFACIAL_ANTI_ACNE: '/images/services/hydrofacial/hydrofacial-in-calgary-004.webp',
  
  // Head spa services
  HEAD_SPA_RELAX: '/images/services/japanese-head-spa/japanese-head-spa-in-calgary-001.webp',
  HEAD_SPA_BOGO: '/images/services/japanese-head-spa/japanese-head-spa-in-calgary-002.webp',
};
 ...
 */
export const downtownGeneralOffers: OfferItem[] = consolidatedOffers.filter(offer => 
  offer.availableLocations?.some(loc => loc.location === "Downtown")
).map(offer => ({
  ...offer,
  // Override with Downtown-specific URL
  url: offer.availableLocations?.find(loc => loc.location === "Downtown")?.url || offer.url,
  // Merge location-specific badges
  badges: [
    ...offer.badges,
    ...(offer.availableLocations?.find(loc => loc.location === "Downtown")?.badges || [])
  ]
}));
 ...
 */
export const edmontonTrailOffers: OfferItem[] = consolidatedOffers.filter(offer => 
  offer.availableLocations?.some(loc => loc.location === "Edmonton Trail")
).map(offer => ({
  ...offer,
  // Override with Edmonton Trail-specific URL
  url: offer.availableLocations?.find(loc => loc.location === "Edmonton Trail")?.url || offer.url,
  // Merge location-specific badges
  badges: [
    ...offer.badges,
    ...(offer.availableLocations?.find(loc => loc.location === "Edmonton Trail")?.badges || [])
  ]
}));
