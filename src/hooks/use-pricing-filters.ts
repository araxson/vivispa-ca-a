...
const usePricingFilters = () => {
  ...
  const locations = useMemo(() => {
    return [...new Set(allLocations.map(location => location.location))].sort();
  }, []);
  ...
  const activeFilters: FilterItem[] = useMemo(() => {
    ...
    if (selectedPriceRange !== 'all') {
      const priceLabel = selectedPriceRange === 'under-100' ? 'Under $100' :
                        selectedPriceRange === '100-200' ? '$100 - $200' :
                        selectedPriceRange === '200-500' ? '$200 - $500' :
                        selectedPriceRange === 'over-500' ? 'Over $500' : selectedPriceRange;
      filters.push({
        type: 'priceRange',
        label: priceLabel,
  ...
  const filteredServices = useMemo(() => {
    ...
    if (selectedLocation === 'all') {
      ...
      const locationData = allLocations.find(loc => loc.location === selectedLocation);
      if (locationData) {
        services = locationData.categories.flatMap(category => category.services);
      }
    }
    ...
    const sortedGrouped = Object.entries(grouped)
      ...
      .reduce((acc, [category, subcategories]) => {
        acc[category] = Object.entries(subcategories)
          .reduce((subAcc, [subcategory, services]) => {
